...
`StdUsage` remains a source-oriented research utility, and the versioned JSON exporter remains a secondary interchange adapter rather than a version of `StdDocument`. Its schema name is `spice_std_json_export` and its independent numeric `schemaVersion` is 6. Version 6 corrects the SPARC field layout and replaces misleading field names; see [SPARC source layout](SparcSourceLayout.md). Cross-file links to MLD, combatants, actions, or other systems are consumer-owned. Additional payload semantics await evidence and consumer feedback before the mutable contract is considered for freezing.
