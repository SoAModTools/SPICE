...
SPARC exposes its scalar and vector region through +0x5F. Its +0x08 word is a decimal-packed MLD filename key, and a formatting helper returns the intrinsic E-family or M-family filename when representable. Asset existence is intentionally consumer-owned. The fixed region from +0x60 through +0x15F is 64 signed `{ value, weight }` choices; the separate signed fields at +0x160/+0x162 are retained as `choiceFirstRaw`/`choiceLastRaw`. Every physical slot remains editable and serializable. SPARC is relocatable, endian-portable, and does not require a receipt. See [SPARC source layout](SparcSourceLayout.md) for the corrected offsets, API changes, and qualification limits.
